...
class Ride(Document):
	def validate(self):
		self.check_minimum_cost_for_petrol()

	def after_insert(self):
		self.update_order_status()

	def check_minimum_cost_for_petrol(self):
		...
		for row in self.cost_breakup:
			if row.item == "Petrol" and cint(row.amount) < minimum_cost_for_petrol:
				frappe.throw(_("Cost of petrol cannot be less than {0}.").format(minimum_cost_for_petrol))
